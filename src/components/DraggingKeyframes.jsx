...
export default class DraggingKeyframes extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      datapoints: null,
  ...
  dragged(d) {
    d3.select(this)
      .attr("cx", (d.time = d3.event.x))
      .attr("cy", (d.intensity = d3.event.y));
  }
  dragended(d) {
  ...
  render() {
    return (
      <svg
        ...
        ref={"pattern"}
      >
        <path d={this.state.pattern} fill="none" stroke={"orange"}></path>
        <g>
          <g
            ref={"xAxis"}
            transform={
